...
export const AuthContextProvider = ({ children }) => {
    ...
    const [role, setCode] = React.useState(null);
    const [remove, setRemove] = React.useState(0);

    const login = React.useCallback((userId, token, username, role) => {
        setUserId(userId);
        setToken(token);
        ...
        localStorage.setItem('storageKey', JSON.stringify({ userId, token, username, role }));
    }, []);
